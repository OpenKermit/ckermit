...
#if defined(__x86_64)
# define CKVFIO_OS_ARCH_STRING " OpenVMS(tm) x86_64"
...
#    else /* def __GNUC__ */
#     ERROR -- CKVFIO.C unknown architecture - Not ALPHA, IA64, VAX, or x86_64
#    endif /* def __GNUC__ */
#   endif /* def VAX */
...
char *ckzv = "File support, 10.0.185, 4 October 2022";
 ...
 *
 *    Kermit I/O currently seems insensitive to mbc and mbf.
 *
 *    This code was adapted from an access callback routine used in the
 *    Info-ZIP programs, and that's what forced the DEC C requirement.
 *    Practically, the GETJPI stuff will fail even with older DEC C
 *    and/or VMS versions, so it makes some sense to retain the DEC C
 *    conditionality.
 */
...
/* JPI item buffers. */
static unsigned short rms_deq;
static char rms_mbc;
static unsigned char rms_mbf;

/* Active RMS item values. */
unsigned short rms_deq_active;
char rms_mbc_active;
unsigned char rms_mbf_active;

/* GETJPI item lengths. */
static int rms_deq_len;         /* Should come back 2. */
...
struct
{
    jpi_item_t rms_deq_itm;
    jpi_item_t rms_mbc_itm;
    jpi_item_t rms_mbf_itm;
    int term;
} jpi_itm_lst =
 { { 2, JPI$_RMS_EXTEND_SIZE, &rms_deq, &rms_deq_len },
...
int get_rms_defaults()
{
    ...
    debug(F111, "rms defaults, sts", "", sts);
    ...
    if (rms_defaults_known > 0)
    {
        ...
        rms_deq_active = RMS_DEQ_DEFAULT;
        ...
        if (rms_deq > 0)
        {
            rms_deq_active = rms_deq;
            debug(F111, "deq =", "", rms_deq);
        }
...
/*  I S A B S O L U T E  --  Tell if a pathname is absolute (not relative)
 *
 * Returns:
 *   0 if relative (or invalid) path
 *   1 if absolute path
 *
 * 2010-03-22 SMS.
 ...
 *
 * Old (ckcmai.c) code said relative was anything containing no dev and
 * no dir or "[." (or "<.").  New code here also includes "[-]" and "[]"
 * as relative.  And, it skips caret-escaped ODS5 extended name
 * characters.
 */
int
isabsolute(path) char * path; {
    ...
    {
        if (path[ i] == '^')            /* Skip caret-escaped chars. */
        {
            i++;
        }
        else if (path[ i] == ':')       /* Record colon. */
        {
            colon = i;
        }
        else if ((path[ i] == '[') || (path[ i] == '<'))
        {
            /* We call "[." or "[-" or "[]" relative. */
            rc = 0;
            debug(F111, "isabsolute left bracket", path, i);
            if ((path[ i+ 1] != '.') && /* Worst case: [i+1] is NUL. */
                (path[ i+ 1] != '-') &&
                (path[ i+ 1] != (path[ i]+ 2)))
            {
                rc = 1;
            }
            break;                      /* In any case, we know now. */
        }
    }
    if (rc < 0)
    {
        ...
        if (colon > 0)
        {
            /* Colon (device or logical) with no directory. */
            /* We treat both as absolute, although this may make little
             * sense for a directory-free device (or logical) name.
             */
            debug(F111, "isabsolute logical", path, colon);
            rc = 1;
        }
    }
...
isdir(path) char *path; {
    ...
    path_nam.NAMX_L_ESA = (char *) &path_exp_name;
    path_nam.NAMX_B_ESS = sizeof( path_exp_name);
    path_nam.NAMX_L_RSA = (char *) &path_res_name;
    path_nam.NAMX_B_RSS = sizeof( path_res_name);
...
iswild(path) char *path; {
    ...
    /* Note that we also have these specific flags available:
     *     nam$v_wild_ver  : Version contained a wild card.
     *     nam$v_wild_type : Type contained a wild card,
     *     nam$v_wild_name : Name contained a wild card.
     *     nam$v_wild_dir  : Directory spec included a wild card.
     *     and a host of others.
     * This one is claimed to be "inclusive or of other wild card bits".
     */
    return path_nam.NAMX_V_WILDCARD;
}


#ifdef COMMENT          /* Replaced by sys$parse() code, above. */
int
iswild(str) char *str; {
    char c;
    if (!str) return(0);
...
*/
        ...
        if (rms_defaults_known > 0)
        {
#if 0       /* These appear to be useless here. */
            rab_ofile.rab$b_mbf = rms_mbf_active;
            rab_ofile.rab$v_rah = 1;
            rab_ofile.rab$v_wbh = 1;
#endif /* 0 */

            /* Default extend quantity. */
            fab_ofile.fab$w_deq = rms_deq_active;

            /* "Sequential access only" helps write with high-water marking. */
            fab_ofile.fab$v_sqo = 1;
        }
#endif /* def __DECC */
...
/* zchout */ {
#ifdef DEBUG
    debug(F101,"zchout","",n); /* SMSd. */
    debug(F101,"zchout fp[n]","", (int)fp[n]); /* SMSd. */
    if (chkfn(n) < 1) return(-1);
#endif
    if (n == ZSFILE) {
...
zoutdump_ascii:
    ...
    for (optr = srcptr; optr < endptr; optr++) if (*optr == CK_CR) break;
